...
function eventHandler() {
    if (percent(demicm.EVENT_MAN_PER)) {
        writeFileCM("fuzzlog",'id_' + (demicm.evtId + demicm.SPEC_OFFSET) + ' = event;');
...
function appendNetwork(rId, rTxt) {
    // Add WebSocket
    writeFileCM("fuzzlog",'id_' + id.length + ' = new WebSocket("ws://127.0.0.1:8080", "' + rTxt + '");');
    ...
    writeFileCM("fuzzlog",'id_' + xhrId + '.open("GET", "http://127.0.0.1:8080", true);');
