...
function appendNetwork(rId, rTxt) {
    // Add WebSocket
    writeFileIE("e://fuzzlog.txt",'id_' + id.length + ' = new WebSocket("ws://127.0.0.1:8080", "' + rTxt + '");');
    ...
    writeFileIE("e://fuzzlog.txt",'id_' + xhrId + '.open("GET", "http://127.0.0.1:8080", true);');
...
function clear() {
    if (demicm.IS_DEBUG) {
        writeFileIE("e://fuzzlog.txt",'[+] clear()');
    }
    ...
    try {
        ...
        switch (rand(8)) {
            ...
            default:
                writeFileIE("e://fuzzlog.txt",'// Warning: clear default');
                break;
        }
        ...
        writeFileIE("e://fuzzlog.txt",'// Error: clear: ' + e.message);
    }
}
