/*
  ...
  - remove all dependencies of console element
*/
...
// Operate number
demicm.FRONT_OP_CNT = 30; // 30
demicm.BACK_OP_CNT = 20; // 20

demicm.EVENT_OP_CNT = 10; // 10
...
demicm.CLEAR_PER = 10; // 15
demicm.CLEAR_ALL_PER = 3; // 5
demicm.DOM_PER = 60;
demicm.GC_PER = 20;
...
demicm.EVENT_CLEAR_PER = 60; // 80
demicm.EVENT_CLEAR_ALL_PER = 20; // 30 
...
function constructNodeIterator() {
    try {
        ...
        console.log("ni= document.createNodeIterator(document.all[" + rb + "], NodeFilter.SHOW_ALL, null, false);");
        ni = document.createNodeIterator(document.all[rb], NodeFilter.SHOW_ALL, null, false);
    }
    catch (exception) {
        console.log( "//Errorin creating iterator in document ");
    }
...
function constructTreeWalker() {
    try {
        rb = rand(document.all.length);
        console.log("tw = document.createTreeWalker(document.all[" + rb + "], NodeFilter.SHOW_ALL, null, false);");
        tw = document.createTreeWalker(document.all[rb], NodeFilter.SHOW_ALL, null, false);
    }
    catch (exception) {
        console.log( "//Errorin creating tree walker in document ");
    }
}

function constructElemRange() {
    try {
        console.log("range = document.createRange();");
        range = document.createRange();

        setElemRange();
    }
    catch (exception) {
        console.log( "//Error in creating range  in document ");
    }
}
...
function setElemRange() {
    try {
        ...
        console.log("var startNode=document.all["+r1+"];");
        var startNode=document.all[r1];

        console.log("range.setStart(startNode, 0);");
        range.setStart(startNode, 0);

        r2=rand(document.all.length);
        console.log("var endNode=document.all["+r2+"];");
        var endNode=document.all[r2];

        console.log("range.setEnd(endNode,0);");
        range.setEnd(endNode, 0);

        console.log("startNode=null;");
        console.log("endNode=null;");
    }
    catch (exception) {
        console.log( "//Error in creating range  in document ");
    }
...
function constructSelection() {
    try {
        ...
        console.log( "//Error in creating selection");
    }
}
...
function setSelection() {
    try {
        rb = rand(document.all.length);
        console.log( "range.selectNodeContents(document.all[" + rb + "]);");
        ...
        console.log( "//Error in setting selection");
    }
}
...
function nodeIteration() {
    try {
        // Fuzz current node
        console.log('currentNodeR = ni.referenceNode;');
        ...
        console.log('currentNodeR = ni.root;');
        currentNodeR = ni.root;
        while (currentNodeR)  {
            propfMan([0], Infinity, 'prop', 'currentNodeR');
            propfMan([0], Infinity, 'func', 'currentNodeR');

            console.log('currentNodeR = ni.nextNode();');
            currentNodeR = ni.nextNode();
        }
    }
    catch (exception) {
        console.log( "//Exception in nodeiteration in document at node "+currentNodeR+"");
    }
}
...
function treeIteration() {
    try {
        // Fuzz current node
        console.log('currentLeafR = tw.currentNode;');
        ...
        // Iterate from root to end
        console.log('currentLeafR = tw.root;');
        currentLeafR = tw.root;
        while (currentLeafR)  {
            propfMan([0], Infinity, 'prop', 'currentLeafR');
            propfMan([0], Infinity, 'func', 'currentLeafR');

            console.log('currentLeafR = tw.nextNode();');
            currentLeafR = tw.nextNode();
        }
    }
    catch (exception) {
        console.log( "//Exception in treeIteration in document at leaf "+currentLeafR+"");
    }
}
...
function moveIterator() {
    try {
        ...
        for (i=0; i<rMoves; i++) {
            ...
            switch(rm) {
                case 0:
                    ...
                    break;
                case 1:
                    console.log( "currentNodeR = ni.previousNode();");
            ...
            switch(rm) {
                case 0:
                    console.log( "cacheValR = ni.filter;");
                    cacheValR = ni.filter;
                    break;
                case 1:
                    console.log( "ni.expandEntityReferences;");
                    ni.expandEntityReferences;
                    break;
                case 2:
                    console.log( "cacheValR = ni.referenceNode;");
                    cacheValR = ni.referenceNode;
                    break;
                case 3:
                    console.log( "ni.pointerBeforeReferenceNode;");
                    ni.pointerBeforeReferenceNode;
                case 4:
                    console.log( "cacheValR = ni.root;");
                    cacheValR = ni.root;
                case 5:
                    console.log( "ni.whatToShow;");
                    ni.whatToShow;
                    break;
            }
        }	
        ...
        console.log( "//Exception in moveIteration ");
    }
}
...
function moveTreeWalker() {
    try {
        ...
        for (i=0; i<rMoves; i++) {
            ...
            switch(rm) {
                case 0:
                    ...
                    break;
                case 1:
                    console.log( "currentLeafR = tw.previousNode();");
                    currentLeafR = tw.previousNode();
                    break;
                case 2:
                    console.log( "currentLeafR = tw.previousSibling();");
                    currentLeafR = tw.previousSibling();
                    break;
                case 3:
                    console.log( "currentLeafR = tw.nextSibling();");
                    currentLeafR = tw.nextSibling();
                    break;
                case 4:
                    console.log( "currentLeafR = tw.firstChild();");
                    currentLeafR = tw.firstChild();
                    break;
                case 5:
                    console.log( "currentLeafR = tw.lastChild();");
                    currentLeafR = tw.lastChild();
                    break;
                case 6:
                    console.log( "currentLeafR = tw.parentNode();");
            ...
            switch(rm) {
                case 0:
                    console.log( "cacheValR = tw.root;");
                    cacheValR = tw.root;
                    break;
                case 1:
                    console.log( "tw.whatToShow;");
                    tw.whatToShow;
                    break;
                case 2:
                    console.log( "cacheValR = tw.filter;");
                    cacheValR = tw.referenceNode;
                    break;
                case 3:
                    console.log( "tw.expandEntityReferences;");
                    tw.expandEntityReferences;
                    break;
                case 4:
                    console.log( "cacheValR = tw.currentNode;");
        ...
        console.log( "//Exception in moveTreeWalker ");
    }
}		
    ...
    try {
        ...
        switch(rm) {
            case 0:
                ...
                break;
            case 1:
                console.log( "cacheValR = range.commonAncestorContainer;");
                cacheValR = range.commonAncestorContainer;
                break;
            case 2:
                console.log( "cacheValR = range.endContainer;");
            ...
            case 3:
                ...
                break;
            case 4:
                console.log( "cacheValR = range.startContainer;");
        ...
        rb=rand(document.all.length);
        console.log( "var randElem=document.all["+rb+"];");
        ...
        switch (rm) {
            ...
            case 1:
                ...
                break;
            case 2:
                console.log( "var documentFragment = range.cloneContents();");
                var documentFragment = range.cloneContents();

                console.log( "randElem.appendChild(documentFragment);");
                randElem.appendChild(documentFragment);

                console.log( "documentFragment = null;");
                break;
            case 3:
                console.log( "var documentFragment = range.extractContents();");
            ...
            case 4:
                ...
                console.log( "var documentFragment = range.createContextualFragment('" + rHTMLCode + "');");
                ...
                break;
            case 5:
                console.log( "range.surroundContents(randElem);");
                range.surroundContents(randElem);
                break;
            case 6:	
                console.log( "range.selectNode(randElem);");
                range.selectNode(randElem);
                break;
            case 7:
                console.log( "range.selectNodeContents(randElem);");
                range.selectNodeContents(randElem);
                break;
            case 8:
                console.log( "range.insertNode(randElem);");
                range.insertNode(randElem);
                break;
            case 9:
                console.log( "range.setStartAfter(randElem);");
                range.setStartAfter(randElem);
                break;
            case 10:
                console.log( "range.setStartBefore(randElem);");
                range.setStartBefore(randElem);
                break;
            case 11:
                console.log( "range.setEndAfter(randElem);");
                range.setEndAfter(randElem);
                break;						
            case 12:
                console.log( "range.setEndBefore(randElem);");
                range.setEndBefore(randElem);
                break;
            case 13:
                console.log( "range.compareNode(randElem);");
                range.compareNode(randElem);
                break;
            case 14:
                console.log( "range.comparePoint(" + rNum + ");");
                range.comparePoint(rNum);
                break;
            case 15:
                console.log( "cacheValR = range.getBoundingClientRect();");
                cacheValR = range.getBoundingClientRect();
                break;
            case 16:
                console.log( "cacheValR = range.getClientRects();");
                cacheValR = range.getClientRects();
                break;
            case 17:
                console.log( "range.intersectsNode(randElem);");
                range.intersectsNode(randElem);
                break;						
            case 18:
                console.log( "range.isPointInRange(randElem, " + rNum + ");");
            ...
            case 19:
                ...
                break;						
            case 20:
                console.log( "var rangeCache = range.cloneRange();");
                var rangeCache = range.cloneRange();

                var type = randItem(['START_TO_START', 'END_TO_END', 'START_TO_END', 'END_TO_START']);
                console.log( "range.compareBoundaryPoints(Range." + type + ", rangeCache);");
        ...
        console.log( "randElem = null;");
    }
    catch( exception ) {
        console.log( "//Exception in alterElemRange function");
    }
}
...
function alterSelection() {
    try {
        ...
        switch(rm) {
            case 0:
                console.log( "cacheValR = selection.anchorNode;");
            ...
            case 1:
                ...
                break;
            case 2:
                console.log( "cacheValR = selection.focusNode;");
        ...
        switch (rm) {
            case 0:
                console.log( "selection.collapse(randElem, " + rNumS + ");");
                selection.collapse(randElem, rNumS);
                break;
            case 1:
                console.log( "cacheValR = selection.getRangeAt(0);");
                cacheValR = selection.getRangeAt(0);
                break;
            case 2:
                console.log( "selection.extend(randElem, " + rNum + ");");
            ...
            case 3:
                ...
                console.log( "selection.modify('" + alter + "', '" + direction + "', '" + granularity + "');");
            ...
            case 5:
                ...
                break;
            case 6:	
                console.log( "selection.selectAllChildren(randElem);");
            ...
            case 9:
                var aPart = randBool();
                console.log( "selection.containsNode(randElem, " + aPart + ");");
                selection.containsNode(randElem, aPart);
                break;
            case 10:
                console.log( "selection.deleteFromDocument();");
                selection.deleteFromDocument();
                break;
            case 11:
                var langRTL = randItem(['PR_TRUE', 'PR_FALSE']);
                console.log( "selection.selectionLanguageChange(" + langRTL + ");");
                eval("selection.selectionLanguageChange(" + langRTL + ");");
                break;						
            case 12:
        ...
        console.log( "//Exception in alterSelection function");
    }
}
...
function reuseRange() {
    nodeIteration();
    try {
        console.log( "ni.detach();");
        ni.detach();
    } 
    catch( exception ) {
        console.log( "//Exception in reuseRange function");
    }
    ...
    for(var i=0; i<demicm.EVENT_ELEM_NUM; i++) {
        ...
        try {
            for(var j=0; j<demicm.EVENT_NUM; j++) {
                ...
                id[setEvtId][rEvt] = new Function('console.log("//id_' + setEvtId + '[\'' + rEvt + '\'] = function()");');
            }
        }
        catch(err) {
...
function funcMan(fuzzObj, fuzzObjStr, logObjStr, func, bNormalFunc, rIds, retValDepth, objName) {
    try {
        ...
        if(objName) {
            console.log('var retValue = ' +  fuzzObjStr + '["' + func + '"](' + paramStr.replace(new RegExp('id\\[', 'gm'), 'id_').replace(new RegExp('\\]', 'gm'), '') + ');');
        } else {
